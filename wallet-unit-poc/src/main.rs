...
use spartan2::{
  provider::T256HyraxEngine, traits::{circuit::SpartanCircuit, snark::R1CSSNARKTrait, Engine}, R1CSSNARK
};
...
impl SpartanCircuit<E> for JWTCircuit {
  fn synthesize<CS: ConstraintSystem<Scalar>>(
    ...
    let witness: Vec<_> = generate_witness_from_wasm(witness_dir, witness_input_json, PathBuf::from("output.wtns"));
  ...
  fn public_values(&self) -> Result<Vec<<E as spartan2::traits::Engine>::Scalar>, SynthesisError> {
        Ok(vec![])
    }
  ...
  fn shared<CS: ConstraintSystem<<E as spartan2::traits::Engine>::Scalar>>(
        &self,
        cs: &mut CS,
      ) -> Result<Vec<bellpepper_core::num::AllocatedNum<<E as spartan2::traits::Engine>::Scalar>>, SynthesisError> {
        Ok(vec![])
    }
